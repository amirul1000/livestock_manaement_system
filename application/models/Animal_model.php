...
class Animal_model extends CI_Model
{
    ...
    function get_animal($id){
		...
		if(!(array)$result){
			$fields = $this->db->list_fields('animal');
	...
	function get_count_animal(){
	 ...
	 /** Get all users-animal
	 *
     */
    function get_all_users_animal(){
        $this->db->order_by('id', 'desc');
	...
	/** Get limit users-animal
	 *@param $limit - limit of query , $start - start of db table index to get query
	 *
     */
    function get_limit_users_animal($limit, $start){
		$this->db->order_by('id', 'desc');
        $this->db->limit($limit, $start);
		$this->db->where('users_id', $this->session->userdata('id'));
        $result = $this->db->get('animal')->result_array();
    ...
    /** Count users-animal rows
	 *
     */
	function get_count_users_animal(){
	   $this->db->where('users_id', $this->session->userdata('id'));
       $result = $this->db->from("animal")->count_all_results();
	   $db_error = $this->db->error();
		if (!empty($db_error['code'])){
